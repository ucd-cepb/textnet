  ...
  if(draw_edges==TRUE){
      if(!parsefileexists){
        ...
        parsedtxt <- spacy_parse(single_plan_text,
                                 ...
                                 nounphrase = T)
        if(mini_data==F){
          saveRDS(parsedtxt, paste0(fpath,"/parsed_",gspids[m]))
        }else{
          saveRDS(parsedtxt, paste0(fpath,"/parsed_mini_",gspids[m]))
        }
      }
      print(paste0("parsing complete: ",gspids[m]))
      
      if(mini_data==F){
        parsedtxt <- readRDS(paste0(fpath,"/parsed_",gspids[m]))
      }else{
        parsedtxt <- readRDS(paste0(fpath,"/parsed_mini_",gspids[m]))
      }
