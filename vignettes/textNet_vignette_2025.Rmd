...
```{r pak, eval=F}
pak::pak("ucd-cepb/textnet@make_trf_gpu")
```
...
for(m in 1:length(old_new_parsed)){
  extracts[[m]] <- textnet_extract(old_new_parsed[[m]],cl=1,
...
```{r parse, eval = T}
#### RECOMMENDED: CREATE A CONDA ENVIRONMENT FOR OPERATING SPACY, INSTALLING SPACY MODELS
reticulate::use_condaenv("spacy-env", required = TRUE)
ret_path <- reticulate::conda_python("spacy-env")
