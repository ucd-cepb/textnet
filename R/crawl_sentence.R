...
crawl_sentence <- function(s){
  ...
  # Try C++ implementation, fall back to R if unavailable (e.g. in forked parallel workers)
  result <- tryCatch(
    crawl_sentence_cpp(
      pos = as.character(s$pos),
      dep_rel = as.character(s$dep_rel),
      head_token_id = as.integer(s$head_token_id),
      lemma = as.character(s$lemma),
      token = as.character(s$token),
      token_id = as.integer(s$token_id),
      tag = as.character(s$tag)
    ),
...
# Pure R implementation of crawl_sentence (fallback for parallel workers)
crawl_sentence_r <- function(s) {
  sentence <- s
