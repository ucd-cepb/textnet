...
for (m in 1:length(gspids)){
  ...
  #create adjacency matrix of unique entities and orgs
  ...
  if(!file.exists(paste0("data/adjmat_orig_",gspids[m]))){
    saveRDS(adj_mat_orig, paste0("data/adjmat_orig_",gspids[m]))
  }
  if(!file.exists(paste0("data/entities_subset_",gspids[m]))){
    saveRDS(entities_subset, paste0("data/entities_subset_",gspids[m]))
  }
  adj_mat_orig <- readRDS(paste0("data/adjmat_orig_",gspids[m]))
  ...
  if(!file.exists(paste0("data/network_maincomponents_",gspids[m]))){
    ...
    for (sent_num in 1:length(sent_by_grp)){
      ...
      for(entity_num in 1:num_entities){
        ...
        if(!(nrow(distinct(entities[entities$entity == sent_by_grp[[sent_num]]$entity[entity_num],], doc_id, sentence_id,.keep_all=T))>1)){
    ...
    tiny_mat <- matrix(rnorm(15),ncol=3)
    rownames(tiny_mat) = c("a","b","c","d","e")
    colnames(tiny_mat) = c("c","e","f")
    
    names <- rownames(tiny_mat)
    tiny_mat[rownames(tiny_mat)!=names[3],colnames(tiny_mat)!=names[3]]
    tiny_mat[rownames(tiny_mat)!="c",colnames(tiny_mat)!="e"]
    adj_mat[rownames(adj_mat)==agencies[q],]
...
#    #}
    ...
    identical(network_entities$entity, name)
    network::set.vertex.attribute(agency_net, "type", network_entities$entity_type)
    saveRDS(agency_net, paste0("data/network_",gspids[m]))
